...
// GET /api/servers/:registry_name - Get details for a specific server by registry name
app.get('/api/servers/:registry_name', (req, res) => {
    const registryName = req.params.registry_name;
    const sql = `SELECT id, display_name, registry_name, github_url, language, config_command 
                 FROM servers 
                 WHERE registry_name = ?`;

    db.get(sql, [registryName], (err, row) => {
        if (err) {
            console.error(`Error fetching server with registry name ${registryName}:`, err.message);
            res.status(500).json({ error: "Internal server error", details: err.message });
        } else if (row) {
            console.log(`Fetched server details for registry name: ${registryName}`);
            res.json(row); // Return the full server details
        } else {
            console.log(`Server with registry name ${registryName} not found.`);
            res.status(404).json({ error: `Server with registry name ${registryName} not found.` });
        }
    });
});

// NEW: GET /api/servers/id/:id - Get details for a specific server by database ID
app.get('/api/servers/id/:id', (req, res) => {
    ...
    db.get(sql, [serverId], (err, row) => {
        if (err) {
            console.error(`Error fetching server with ID ${serverId}:`, err.message);
            res.status(500).json({ error: "Internal server error", details: err.message });
        } else if (row) {
            console.log(`Fetched server details for ID: ${serverId}`);
            res.json(row); // Return the full server details
        } else {
...
// NEW: PUT /api/servers/:id - Update an existing server by database ID
app.put('/api/servers/:id', (req, res) => {
    const serverId = req.params.id;
    const { display_name, github_url, language, config_command } = req.body;

    // Basic validation
    if (!display_name || !config_command) {
        return res.status(400).json({ error: "Missing required fields: display_name and config_command." });
    }
    
    // Note: We generally shouldn't allow changing the registry_name easily as it's the lookup key.
    // If display_name changes, the registry_name *could* be regenerated, but this might break things.
    // For now, let's assume registry_name remains fixed after creation.
    // We could potentially regenerate it if the user explicitly requests it via another mechanism.

    // Validate config_command is potentially valid JSON (basic check)
    let parsedConfig;
    try {
        parsedConfig = JSON.parse(config_command);
        // Optional: Add more specific schema validation here if needed
    } catch (e) {
        console.warn(`Invalid JSON submitted for config_command for server ID ${serverId}:`, config_command);
        return res.status(400).json({ error: "Invalid JSON format for 'config_command'.", details: e.message });
    }
    const validConfigCommand = JSON.stringify(parsedConfig, null, 2); // Store pretty-printed version
    ...
    const params = [display_name, github_url, language, validConfigCommand, serverId];

    db.run(sql, params, function(err) {
        if (err) {
            console.error(`Error updating server ID ${serverId}:`, err.message);
            res.status(500).json({ error: "Failed to update server.", details: err.message });
        } else if (this.changes === 0) {
            console.log(`Update requested for non-existent server ID: ${serverId}`);
            res.status(404).json({ error: `Server with ID ${serverId} not found.` });
        } else {
            console.log(`Successfully updated server ID: ${serverId}`);
            // Fetch the updated record to return it
            const fetchSql = `SELECT id, display_name, registry_name, github_url, language, config_command 
                              FROM servers WHERE id = ?`;
            db.get(fetchSql, [serverId], (fetchErr, row) => {
                if (fetchErr) {
                    console.error(`Error fetching updated server ID ${serverId}:`, fetchErr.message);
                    // Still return success, but without the updated object
                    res.json({ message: "Server updated successfully, but failed to fetch updated details." }); 
                } else {
                     res.json({ message: "Server updated successfully.", server: row });
                }
            });
        }
    });
